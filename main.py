...
class App(QMainWindow):
    ...
    def _build(self):
        ...
        g2 = QGroupBox("구간 선택")
        v2 = QVBoxLayout(g2)
        v2.setContentsMargins(10, 8, 10, 8)
        v2.setSpacing(6)
        ...
        grid.addWidget(QLabel("시작:"), 0, 0)
        self.ts = QLineEdit("0:00"); self.ts.setFixedWidth(80)
        self.ts.editingFinished.connect(lambda: self._sync("s"))
        grid.addWidget(self.ts, 0, 1)
        grid.addWidget(QLabel("종료:"), 0, 2)
        self.te = QLineEdit("0:00"); self.te.setFixedWidth(80)
        self.te.editingFinished.connect(lambda: self._sync("e"))
        grid.addWidget(self.te, 0, 3)
        self.dlbl = QLabel("클립 길이: 0초"); self.dlbl.setObjectName("dur")
        grid.addWidget(self.dlbl, 0, 4)
        ...
        sg.addWidget(self.cb_mute, 2, 0, 1, 3)
